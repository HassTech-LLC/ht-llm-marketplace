...
import { evaluateGuard, isAllowedLocalOrigin, isLoopbackHost } from "../server.js";

const config = { host: "127.0.0.1", allowedOrigins: ["http://127.0.0.1:3009", "http://localhost:5173"] };
...
describe("isAllowedLocalOrigin", () => {
  it("allows any loopback origin and configured origins", () => {
    expect(isAllowedLocalOrigin("http://127.0.0.1:3000", config.allowedOrigins)).toBe(true);
...
describe("evaluateGuard", () => {
  ...
  it("allows a state-changing request from the local studio origin", () => {
