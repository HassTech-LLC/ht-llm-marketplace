...
## 📋 7. Audit & Validation Synthesis (June 2, 2026)

We conducted a complete audit of the repository code, test suites, and running web application:
* **UI Persistence & Badge Accuracy**: Verified that left-pane model size tags (`8B • 4.8 GB` vs `8B • ~4.8 GB`) and right-pane glassmorphic `.ht-tab-size-badge` elements correctly resolve real vs estimated file/VRAM footprints.
* **Layout Truncation**: Confirmed that long model names do not clip the right border of the Smart Pick card due to the `flex: 1; min-width: 0;` styling addition.
* **Stale Port & Lockfile Race Conditions**: Documented that running Vite concurrently with the daemon can lead to Vite baking a stale port config from `active-daemon.json` if it starts before the daemon updates the file. Resolved by ensuring the daemon writes the lockfile prior to Vite bundler execution.
* **Testing Integrity**:
